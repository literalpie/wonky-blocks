...
class WonkyRow: SKShapeNode {
    ...
    init(rowNumber: Int = 0) {
        ...
        self.path = CGPath(rect: CGRect(x: 0, y: 50 * rowNumber, width: Self.rowWidth, height: Self.rowHeight), transform: nil)
        physicsBody = SKPhysicsBody(rectangleOf: rowSize, center: CGPoint(x: rowCenter.x, y: rowCenter.y))
    ...
    func calculateRowArea() -> CGFloat {
        ...
        let totalArea = uniqueBodies!.reduce(0, { (soFar, current) -> CGFloat in
