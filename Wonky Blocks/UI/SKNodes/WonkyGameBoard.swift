...
class WonkyGameBoard: SKNode {
    ...
    override init() {
        ...
        leftWall.physicsBody = SKPhysicsBody(edgeFrom: CGPoint(x: 0, y: 0), to: CGPoint(x: 0, y: height))
        rightWall.physicsBody = SKPhysicsBody(edgeFrom: CGPoint(x: width, y: 0), to: CGPoint(x: width, y: height))
